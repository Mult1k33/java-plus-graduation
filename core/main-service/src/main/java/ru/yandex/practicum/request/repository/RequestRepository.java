...
import ru.yandex.practicum.event.dto.EventWithCountConfirmedRequests;
...
public interface RequestRepository extends JpaRepository<Request, Long> {
    ...
    @Query("""
            SELECT NEW ru.yandex.practicum.event.dto.EventWithCountConfirmedRequests(r.event.id, COUNT(r))
            FROM Request r
            WHERE r.event.id IN :eventIds AND r.status = ru.yandex.practicum.request.model.RequestStatus.CONFIRMED
            GROUP BY r.event.id"""
    )
    ...
    @Query("""
            SELECT NEW ru.yandex.practicum.request.dto.ParticipationRequestDto(
                r.id,
                r.created,
                r.event.id,
                r.requester.id,
